...
- `awk` Инструмент для работы с текстовыми файлами `awl '/pattern/{action}' file`
  - команде передается паттерн которому должен соответствовать текст
    далее идет действие которое совершается над найденными соответствиями(заключаются в фигурные скобки)(также шаблон можно помещать в одинарные кавычки, чтобы оболочка не интерпретировала их)
    И в конце идет файл в котором мы ищем соответствия(без шаблона действие выполняется над каждой строкой файла)
  - Пример: имеем файл с содержанием
    - "Tom:39:Microsoft
    - Bob:43:Google
    - Sam:28:Jetbrains
    - Alice:34:Microsoft
    ...
    - Tim:23:-
    - Tomas:31:Apple"
  - Команда`awk '/Tom/' users.txt` выведет все строки где имя пользователя "Tom"
  - `-F` указывает разделитель
  - Пример: `awk -F : '/Tom/{print $3}' users.txt` команда имеет опцию `print` благодаря которой, ей передается значение 3 столбца, выведет только компанию в которой работают пользователи с ником "Tom"
  - Пример: `awk -F : '$2 > 30 {print $1}' users.txt` данная команда выведет имена всех пользователей возраст которых больше 30

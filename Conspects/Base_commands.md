...
- `mount` Используется для монтирования устройства
  - Пример: К компу подключен ssd "/dev/nvme0n1p3", система его не видит тк он не смотрирован
  Для монтирования используется каталог "media" обычно в этом каталоге создается каталог для 
  пользователя и в него происходит автоматическое монтирование
    - Создаем папку для устройства в нашем каталоге для устройства и далее монтируем
    `sudo mount /dev/nvme0m1p3 "путь к папке куда монтируем"`
    Таким образом устройство монтируется в папку и мы можем перейти к диску и просматривать его содержимое
  - Аргументы:
    - `-r` используется для монтирования устройства только для чтения
    - `-w` используется для монтирования устройства только для записи
    - `-t` используется для выбора файловой системы для устройства
- `umount` Команда для отключения устройства, может принимать значения как директории в которую вмонтировано устройство
так и имя устройства (если устройство используется другими процессами их можно завершить с помощью `fuser -km "название устройства"`)
- `lsblk` отображает все устройства доступные в текущий момент на компе
